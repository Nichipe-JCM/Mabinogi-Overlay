...
현재 베타 후보 버전은 `0.0.5-beta`입니다. 버프/투아림 감시, 단축키 타이머, 에린 타이머와 다중 프로필 관리를 포함합니다.
...
설정의 `파일로 저장`은 `.moverlayprofile` 패키지를 만들며, 프로필에서 사용하는 사용자 알림음도 함께 포함합니다. 이전 버전에서 내보낸 `.json` 프로필도 계속 불러올 수 있습니다.

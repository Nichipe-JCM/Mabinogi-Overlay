...
- 기존 폴더에 새 버전을 덮어쓰면 `settings.json`과 `save` 폴더를 그대로 사용합니다.
- 새 폴더에 설치할 때는 기존 `settings.json`과 `save` 폴더를 함께 복사한 뒤, 설정에서 프로필 저장 경로를 새 폴더의 `save`로 다시 지정해야 합니다. 현재 버전은 프로필 경로를 절대 경로로 저장합니다.

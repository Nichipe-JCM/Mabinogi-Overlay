...
현재 버전은 `0.0.6-beta`입니다. 버프/투아림 감시, 단축키 타이머, 에린 타이머와 다중 프로필 관리를 포함합니다.

[0.0.6-beta 릴리스](https://github.com/Nichipe-JCM/Mabinogi-Overlay/releases/tag/0.0.6-beta)에서 Windows x64 portable ZIP을 받아 전체 압축을 풀고 `Mabinogi Overlay.exe`를 실행하세요. .NET 런타임이 포함되어 있습니다.

자세한 내용은 [한국어 변경사항](releases/0.0.6-beta/CHANGELOG.ko.md)을 참고하세요.

## 0.0.6-beta 주요 변경

- DXGI 캡처 자원과 CPU 합성 버퍼를 재사용하고, 데스크톱 캡처 대기를 UI 스레드에서 분리했습니다.
- 캡처 중지·재시작, 오래된 프레임 거부와 화면 밖 오버레이 위치 복구를 보완했습니다.
- 유효한 프로필 백업을 보존하고, 프로필·에린 알람 저장 실패를 표시하며 재시도합니다.
- 중복 실행 시 기존 창을 활성화하고 트레이·레이아웃·모니터 조작을 개선했습니다.
- 단위 테스트 158개와 Windows 빌드 CI를 통과했습니다. 실제 게임·GPU·다중 DPI 환경 검증을 모두 완료했다는 뜻은 아닙니다.

배포 실행 파일에는 임시 자체 서명 인증서와 타임스탬프를 사용합니다. Windows에서 신뢰 경고가 표시될 수 있습니다. 도움말 이미지는 다음 버전으로 미뤘으며, 텍스트 도움말은 그대로 사용할 수 있습니다.

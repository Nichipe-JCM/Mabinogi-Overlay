...
현재 공개 기준 버전은 `0.0.3-beta`입니다. 버프/투아림 감시와 에린 타이머는 현재 개발 브랜치에서 확장 중인 기능입니다.
...
- GPU/DXGI, CPU/Composited, CPU/WPF 오버레이 렌더러 선택
...
- **WGC:** 기본 캡처 방식이며 GPU/DXGI 렌더러의 입력으로 사용됩니다. 지원되는 Windows에서는 테두리 없는 캡처 권한을 요청하고, 허용되면 노란 캡처 테두리를 숨깁니다.
- **DXGI:** 선택 창이 있는 모니터를 캡처한 뒤 게임 클라이언트 영역을 자릅니다. 개선 CPU/합성 렌더러를 사용하며, 다른 창이 게임 위를 가리면 그 화면도 반영될 수 있습니다.
- **GDI:** 호환성용 데스크톱 캡처 방식입니다.

## 저장 위치

기본 portable 구성에서는 실행 파일과 같은 폴더에 다음을 저장합니다.

- `settings.json`: 언어, 캡처 방식, 렌더러, 프로필 저장 경로
- `save/<프로필명>.json`: 후보, 섹션, 레이아웃, 감시 설정
- `Logs/app.log`: 현재 앱 세션 로그

설정에서 프로필 저장 경로는 별도로 변경할 수 있습니다.

...
- **WGC:** 선택한 게임 창을 직접 캡처합니다. 설정 단계에서는 가장 정확하지만 Windows 환경에 따라 노란 캡처 테두리가 보일 수 있습니다.
- **DXGI:** 선택 창이 있는 모니터를 캡처한 뒤 게임 클라이언트 영역을 자릅니다. 기본값이며 테두리가 없지만 다른 창이 게임 위를 가리면 그 화면도 반영될 수 있습니다.

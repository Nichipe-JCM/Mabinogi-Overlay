    ...
    private async Task SynchronizeMonitorValuesAsync(string reason)
    {
        ...
        try
        {
            ...
            if (shouldReadBuffs && _buffMonitorRoi is Rect buffRoi)
            {
                ...
                var batch = await _monitorValueRecognition.ReadBuffTimesAsync(
                    frame,
                    buffRoi,
                    activeMatches,
                    cancellationToken);
                ...
                foreach (var activeMatch in activeMatches)
                {
                    ...
                    if (!batch.Reads.TryGetValue(nameKey, out var read))
                    {
                        usedFallback = true;
                        fallbackCount++;
                        read = await _monitorValueRecognition.ReadBuffTimeAsync(
                            frame,
                            buffRoi,
                            activeMatch.Bounds,
                            cancellationToken);
            ...
            {
                var read = await _monitorValueRecognition.ReadTuairimPercentAsync(
                    frame,
                    tuairimAnchor,
                    cancellationToken);
                if (generation != _monitorValueRecognitionGeneration || !_overlayRuntime.IsRunning)
        ...
        {
            _log.Info($"Monitor value recognition canceled: reason={reason}, generation={generation}.");
        }
